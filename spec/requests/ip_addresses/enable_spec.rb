...
RSpec.describe "POST /ip_address/:id/enable", type: [:request, :db] do
  let(:repo) { IpMonitoring::Repos::IpAddressRepo.new }

  describe "valid requests" do
    context "when the request is valid" do
      let!(:ip_address) { repo.create(ip: Faker::Internet.ip_v4_address, enabled: false, created_at: Time.now, updated_at: Time.now).to_h }
      let(:id) { ip_address[:id] }

      it "enables the IP and returns it as JSON" do
        post "/ip_address/#{id}/enable", {}
